...
<body class="bg-gray-200 flex justify-center items-center h-screen">
    ...
    <!-- <script>
        ...
        console.log('Before subscribing to Pusher channel');
        console.log('Subscribed to Pusher channel: ' + recipientChannel);
        ...
        function tickmark(status) {
            if (status == "sent") {
                $("#chat_area").append('<i class="fas fa-check"></i>');
            } else if (status == "delivered") {
                $("#chat_area").append('<i class="fas fa-check-double"></i>');
            } else {
                $("#chat_area").append('<i class="fas fa-check-double seen"></i>');
            }
        }
        channel.bind('private-message', function(data) {
            ...
            if (senderId == recipientId) {
                let receiverMessage = `
                            <div class="d-flex flex-row p-3">
                                <img src="https://img.icons8.com/color/48/000000/circled-user-male-skin-type-7.png" width="30" height="30">
                                <div class="chat ml-2 p-3">${data.message} <i class="fas fa-check-double"></i></div>
                            </div>`;
            ...
            if (unreadCount.length) {
                console.log("inCount", unreadCount.length);
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    if (chatId) {
                        let send = `
                            <div class="d-flex flex-row p-3">
                                <div class="bg-white mr-2 p-3"><span class="text-muted">${message}</span><i class="fas fa-check"></i></div>
                                <img src="https://img.icons8.com/color/48/000000/circled-user-male-skin-type-7.png" width="30" height="30">
                            </div>`;
                    ...
                    // Update chat list with unread message count if recipient ID is present
                    // var unreadCount = $('.unread-count[data-chat="' + chatId + '"]');
                    // if (unreadCount.length) {
                    //     unreadCount.text(parseInt(unreadCount.text()) + 1);
                    // } else {
    ...
    <!-- <script>
    $(document).ready(function() {
        // Trigger AJAX request to mark messages as read when chat area is loaded
        markMessagesAsRead();
        // console.log();
            ...
            if (chatId) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        console.log('Messages marked as read');
                        tickmark("reached");
                        // Remove the unread count from the UI
                        $('.unread-count[data-chat="' + chatId + '"]').remove();
                    },
                    error: function(xhr, status, error) {
                        console.error(error);
        ...
        function tickmark(status){
            ...
            if (status == "sent") {
            // Update the UI to show a single tick indicating that the message is sent
            $("#chat_area").append('<i class="fas fa-check"></i>');
            } else if (status == "delivered") {
                // Update the UI to show a double tick indicating that the message is delivered
                $("#chat_area").append('<i class="fas fa-check-double"></i>');
            } else {
                $("#chat_area").append('<i class="fas fa-check-double"></i>');
